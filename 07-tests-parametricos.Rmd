...
```{definition, name="Error estándar de diferencia entre medias"}
$$
...
```{definition, name="Error estándar de diferencia entre medias estimado de muestras", label='SED-samples'}
$$
...
```{definition, name="Prueba de t", label='test-t'}
...
```{example, name="Prueba t", label="manual-t-test"}
:
```
...
```{example, name="Prueba t en R",label="t-test-in-r"}
:
```
...
```{example, name="Test de Shapiro Wilks en R"}
:
```
...
```{example, name="Realizar la prueba de Fisher en R"}
:
```
...
```{definition, name="Prueba t para muestras dependientes", label='dependent-t-test'}
...
```{example, name="Prueba t dependiente", label='dependent-t-test-example'}
:
```
