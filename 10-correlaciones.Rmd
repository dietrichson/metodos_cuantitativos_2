...
```{r strong-positive-correlation, echo=FALSE, fig.cap="Correlación positiva"}
...
```{r strong-negative-correlation, echo=FALSE, fig.cap="Correlación negativa"}
...
```{r weak-negative-correlation, echo=FALSE, fig.cap="Correlación negative leve"}
...
```{r near-perfect-negative-correlation, echo=FALSE, fig.cap="Correlación casi perfecta"}
...
```{r no-correlation, echo=FALSE, fig.cap="Correlación nula"}
...
```{r non-linear-relationship, echo=FALSE, fig.cap="Relación no lineal"}
set.seed(1)
...
```{example, label="example-scatterplot-i", name="Generar diagrama de dispersión en R"}
:
```
...
```{example, label="example-scatterplot-ii", name="Generar diagrama de dispersión en R"}
:
```
...
```{definition, label="definition-pearson-correlation", name="Coeficiente de correlación de Pearson"}
:
```
$$
...
```{definition, label="definition-pearson-correlation-ii", name="Coeficiente de correlación Pearson"}
:
```
...
```{example, label="pearson-coefficient-calculation", name="Cálculo del Coeficiente de correlación de Pearson"}
:
```
...
```{r resultados-dos-examenes, echo=FALSE}
datos %>% 
  knitr::kable(
...
```{r resultados-dos-examenes-ii, echo=FALSE}
datos <- datos %>% 
...
```{example, label="pearson-coefficient-calculation-in-r", name="Cálculo del Coeficiente de correlación de Pearson en R"}
:
```
...
```{definition, label="definition-spearman-correlation", name="Coeficiente de correlación de Spearman"}
:
```
$$
...
```{r two-by-two-general, echo = FALSE, message=FALSE,warning=FALSE, fig.width=5, fig.height=2.5  ,fig.cap="Tabla de contingencia dos por dos"}
...
```{definition, label="phi-correlation", name="Coefficiente de $\\phi$"}
$$
