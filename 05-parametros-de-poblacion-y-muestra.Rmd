...
```{example, name='Distribucion de muestras'}
:
```

```{r muestras-de-estaturas, echo=FALSE,message=FALSE,warning=FALSE, fig.cap="Cinco Muestras de 20 obseraciones"}
library(bayestestR)
library(tidyverse)
...
```{r curva-normal-poblacion-muestral, echo=FALSE, fig.cap='Distribución de la población y la muestra'}
...
```{definition, name = "Error estándar"}
...
```{example, name="Muestra pequeña", label='small-sample'}
...
```{example, name='Ejemplo en R: extraer el valor crítico de t'}
:
```
