...
```{r curva-normal-IV, fig.cap='Curva normal', echo=FALSE}
x <- distribution_normal(n = 100, mean = 0, sd = 1)
x %>% 
  density() %>%  # Compute density function
  as.data.frame() %>% 
...
```{r histograma-ejemplo-i, echo = FALSE, fig.cap='Muestra de notas de un test de matemática (N=1000)'}
...
```{r histograma-ejemplo-ii, echo = FALSE, fig.cap='Muestra de notas de un test de matemática (N=1000)'}
...
```{r histograma-ejemplo-iii, echo = FALSE, fig.cap='Muestra de notas de un test de matemática (N=1000)'}
...
```{r normal-curve-with-cuts,echo=FALSE, warning=FALSE, fig.cap='Área debajo de la curva normal'}
x <- distribution_normal(n = 100, mean = 0, sd = 1)
x %>% 
  density() %>%  # Compute density function
  as.data.frame() ->tmp
...
```{definition, label='definition-z-score', name='Variable normalizada'}
...
```{example, name = "Analizando datos del ministerio de salud", label='estatura-mujeres-argentinas'}
:
```
...
```{r curva-estatura-argentinas, echo=FALSE, fig.cap="Estatura de argentinas entre 19 y 49 años"}
...
```{r curva-con-segmento, echo=FALSE, fig.cap="Proporción de argentinas que miden más de 175 centímetros"}
plot_estatura+
